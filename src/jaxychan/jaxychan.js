...
var jaxy = {
    ...
    start: function() {
        ...
        setInterval(jaxy.loadThreads, 60000);
    },
    clearDom: function() {
        $("body").html([
           "<div class='sidebar'>Ur Chan</div>",
           "<div class='posts'></div>",
           "<style type='text/css'>",
                ".sidebar {",
                    "height: 100%;",
                ...
                ".name {",
                    ...
                    "color: black;",
                "}",
                "a.replydot:link, a.replydot:visited {",
                    "color: black;",
                ...
                "}",
           "</style>"].join(""));
        document.title = "I'm in ur chan, jaxing ur posts";
        jaxy.domCleared = true;
    },
    loadThreads: function() {
        if (jaxy.board) {
            // reload dom if we've already been here
            $.get("http://boards.4chan.org/" + jaxy.board + "/", function(data) {
                new ChanBoard($(data));
            });
        } else {
            jaxy.board = new ChanBoard(window.document).board;
        }
    }
};

function ChanBoard(dom) {
    var board = window.location.href.replace("http://", "").split("/")[1];
    var threadIds = [];
    var threads = [];
    $(dom).find("a.quotejs").each(function(i, el) {
        var href = $(el).attr('href');
        var id = href.split('/')[1].split('#')[0];
        if ($.inArray(id, threadIds) == -1) {
            threadIds.push(id);
        }
    });
    var getThread = function() {
        threads.push(new ChanThread(threadIds.shift(), board));
        if (threadIds.length > 0) {
            window.setTimeout(getThread, 500);
        }
    }
    getThread();
...
function ChanThread(id, board) {
    var posts = [];
    var thread = this;
    $.get("http://boards.4chan.org/" + board + "/res/" + id, function(data) {
        var dom = $(data);
        var replies = $(dom).find("td.reply");
        $.each(replies, function(i, td) {
            posts.push(new ChanPost(td, id, true));
        });
        // Strip posts from DOM to get them out of the way.
        $.each(posts, function(i, chanpost) {
            dom.remove("#" + chanpost.threadId);
        });
        // Add the original node.
        for (var i = 0; i < dom.length; i++) {
            if (dom[i].tagName && dom[i].tagName.toLowerCase() == "form") {
                orig = $(dom[i]);
                break;
            }
        }
        posts.splice(0, 0, new ChanPost(orig, id, false));
...
function ChanPost(node, threadId, isReply) {
    this.node = node;
    ...
    this.postNumber = function() {
        var id;
        if (!isReply) {
            return $($(node).find("a")[2]).attr('name')
        } else {
            return $(node).attr('id');
        }
    };
    var _postNumber = this.postNumber();
    this.name = function() {
        var nameNodes;
        if (isReply) {
            nameNodes = $(node).find("span.commentpostername");
        } else {
            nameNodes = $(node).find("span.postername");
        }
    ...
    this.email = function() {
        var mailnode = $(node).find("a.linkmail");
        if (mailnode.length > 0) {
            return $(mailnode).attr("href").replace("mailto:", "");
        } else {
            return null;
        }
    };
    this.creationTime = function() {
        var numberNode;
        if (isReply) {
            numberNode = $(node).find('[id="norep' + _postNumber + '"]');
        } else {
            numberNode = $(node).find('[id="nothread' + _postNumber + '"]');
        }
        var dateString = $(numberNode[0].previousSibling).text();
        return dateString;
    };
    this.tripCode = function() {
        var tripNodes = $(node).find("span.postertrip");
        if (tripNodes.length > 0) {
            return $(tripNodes[0]).text();
        } else {
            return null;
        }
    };
    this.textContent = function() {
        var txts = $(node).find("blockquote");
        if (txts.length == 0) {
            return "";
        }
        return $(txts[0]).html();
    ...
    this.imgData = function() {
        var imgDataNodes = $(node).find("span.filesize");
        var imgData = {
            url: null,
            urlFilename: null,
            sourceFilename: null,
        ...
        if (imgDataNodes.length == 0) {
            return imgData;
        }
        var imgDataNode = $(imgDataNodes[0]);
        imgData.url = imgDataNode.find("a").attr("href");
        imgData.urlFilename = imgDataNode.find("a").text();
        imgData.sourceFilename = imgDataNode.find("span").attr("title");
        var fileInfo = $.trim($(imgDataNode.find("a")[0].nextSibling).text());
        var match = /-\((\d+\.?\d*) (B|KB|MB), (\d+)x(\d+),/.exec(fileInfo);
        if (match) {
            if (match[2] == 'B') {
                imgData.kb = parseFloat(match[1]) / 1024;
            } else if (match[2] == 'MB') {
                imgData.kb = parseFloat(match[1]) * 1024;
            }
            imgData.width = match[3];
            imgData.height = match[4];
        }
        imgData.thumbnail = $(node).find("img").attr("src");
        return imgData;
    };
    this.subject = function() {
        var subjectNodes = $(node).find("span.replytitle");
        if (subjectNodes.length > 0) {
            var text = $(subjectNodes[0]).text();
            if (text) {
                return text;
            }
        }
    ...
    this.render = function() {
        var post = ["<div class='post ", (isReply ? "reply" : "orig"), 
                                " thread", threadId, " post", _postNumber, "'>",
                        "<div class='titleline'>",
                            "<div class='number'><a name='", _postNumber, "'>", _postNumber, "</a></div>",
                            "<div class='date'>", this.creationTime(), "</div>",
                            "<div class='name'>", this.name(), "</div>"];
        var tc = this.tripCode();
        if (tc) {
            post = post.concat(["<div class='trip'>", tc, "</div>"]);
        }
        post = post.concat(["&nbsp;</div>"]);
        post = post.concat(["<table><tr>"]);
        var img = this.imgData();
        if (img.url) {
            post = post.concat([
                        "<td class='image'><img src='", img.thumbnail, "' /></td>"]);
        }
        post = post.concat([
                        "<td class='text'>", this.textContent(), "</td>",
                    "</tr></table>",
    ...
    this.insert = function() {
        var existing = $(".posts").find(".post" + _postNumber);
        if (existing.length > 0) {
            return;
        } else {
            var inserted = false;
            if (isReply) {
                var thread = $(".posts").find(".thread" + threadId);
                var sideThread = $(".sidebar").find(".thread" + threadId);
                if (thread.length > 0) {
                    $(thread[thread.length - 1]).after(this.render());
                    $(sideThread[thread.length - 1]).after(" ",
                        "<a href='#" + _postNumber + "' class='replydot thread" + threadId + " post" + _postNumber + "'>.</a>"
                    );
                    inserted = true;
                }
            }
            if (!inserted) {
                $(".posts").append($(this.render()).fadeIn());
                $(".sidebar").append("<br /><a href='#" + _postNumber + "' class='thread" + threadId + " post" + _postNumber + "'>" + _postNumber + "</a>");
                $(".sidebar").append(" ");
            }
        }
        $(".sidebar").find(".post" + _postNumber).effect(
            "highlight"
        );
    };
}
